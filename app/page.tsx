    ...
    <main className="bg-[#f8f7f3] min-h-screen">
      ...
      <section className="px-6 py-12 md:py-20 border-b border-zinc-200">
        <div className="max-w-7xl mx-auto">
          <Link href={`/articles/${featuredArticle.category}/${featuredArticle.slug}`} className="group grid md:grid-cols-2 gap-12 items-center">
            <div className="overflow-hidden bg-zinc-900">
              <img 
                src={featuredArticle.image} 
                className="aspect-video object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" 
                alt="Featured"
              />
            </div>
            <div>
              <span className="text-[#800020] font-bold uppercase tracking-[0.3em] text-[10px]">Featured Story</span>
              <h2 className="font-serif text-5xl md:text-7xl font-black mt-4 leading-[0.9] tracking-tighter text-[#181717] group-hover:text-[#800020] transition-colors duration-500">
                {featuredArticle.title}
              </h2>
              <p className="mt-6 text-lg text-zinc-600 leading-relaxed italic">
                {featuredArticle.excerpt}
              </p>
              <div className="mt-5 font-bold text-xs uppercase tracking-widest inline-block border-b-2 border-[#181717] text-[#181717]">
                Read More
              </div>
            </div>
          </Link>
      ...
      {/* 2. THE GRID - Artikel Terbaru lainnya */}
          ...
          {otherArticles.map((article) => (
            ...
            >
              <div className="overflow-hidden bg-zinc-900 mb-6">
                <img 
                  src={article.image} 
                  className="aspect-[4/3] object-cover grayscale group-hover:grayscale-0 transition-all duration-500" 
                  alt={article.title}
                />
              </div>
